...
from backend.handlers.shortcut_handler import ShortcutHandler
from backend.handlers.wabbajack_parser import WabbajackParser
import traceback
from backend.core.modlist_operations import get_jackify_engine_path
import signal
import re
import time
from backend.handlers.subprocess_utils import ProcessManager
from backend.handlers.config_handler import ConfigHandler
from ..dialogs import SuccessDialog
from backend.handlers.validation_handler import ValidationHandler
from frontends.gui.dialogs.warning_dialog import WarningDialog
from frontends.gui.services.message_service import MessageService
from backend.utils.nexus_premium_detector import is_non_premium_indicator
# R&D: Progress reporting components
from backend.handlers.progress_parser import ProgressStateManager
from frontends.gui.widgets.progress_indicator import OverallProgressIndicator
from frontends.gui.widgets.file_progress_list import FileProgressList
from shared.progress_models import InstallationPhase, InstallationProgress, OperationType
# Modlist gallery (imported at module level to avoid import delay when opening dialog)
from frontends.gui.screens.modlist_gallery import ModlistGalleryDialog
...
class InstallModlistScreen(QWidget):
    ...
    def __init__(self, stacked_widget=None, main_menu_index=0):
        ...
        from backend.services.protontricks_detection_service import ProtontricksDetectionService
        from backend.handlers.config_handler import ConfigHandler
    ...
    def refresh_paths(self):
        ...
        from shared.paths import get_jackify_logs_dir
    ...
    def _start_gallery_cache_preload(self):
        ...
        class GalleryCachePreloadThread(QThread):
            ...
            def run(self):
                try:
                    from backend.services.modlist_gallery_service import ModlistGalleryService
    ...
    def _check_protontricks(self):
        ...
        try:
            ...
            if not is_installed:
                ...
                from frontends.gui.dialogs.protontricks_error_dialog import ProtontricksErrorDialog
    ...
    def _check_ttw_eligibility(self, modlist_name: str, game_type: str, install_dir: str) -> bool:
        ...
        try:
            ...
            from backend.data.ttw_compatible_modlists import is_ttw_compatible
    ...
    def run_modlist_installer(self, modlist, install_dir, downloads_dir, api_key, install_mode='online', oauth_info=None):
        ...
        from backend.handlers.logging_handler import LoggingHandler
        from pathlib import Path
        ...
        class InstallationThread(QThread):
            ...
            def run(self):
                try:
                    ...
                    from backend.handlers.subprocess_utils import get_clean_subprocess_env
                    ...
                    if self.oauth_info:
                        ...
                        from backend.services.nexus_oauth_service import NexusOAuthService
    ...
    def on_progress_updated(self, progress_state):
        ...
        if is_installation_phase:
            ...
            from shared.progress_models import FileProgress, OperationType
    ...
    def on_installation_finished(self, success, message):
        ...
        if success:
            ...
            from shared.progress_models import InstallationProgress, InstallationPhase
    ...
    def start_automated_prefix_workflow(self):
        ...
        try:
            ...
            class AutomatedPrefixThread(QThread):
                ...
                def run(self):
                    try:
                        from backend.services.automated_prefix_service import AutomatedPrefixService
    ...
    def validate_manual_steps_completion(self):
        ...
        from backend.handlers.shortcut_handler import ShortcutHandler
        from backend.services.platform_detection_service import PlatformDetectionService
        ...
        proton_ok = False
        try:
            from backend.handlers.modlist_handler import ModlistHandler
            from backend.handlers.path_handler import PathHandler
            ...
            class ConfigThread(QThread):
                ...
                def run(self):
                    try:
                        from backend.services.modlist_service import ModlistService
                        from backend.models.configuration import SystemInfo
                        from backend.models.modlist import ModlistContext
                        from pathlib import Path
